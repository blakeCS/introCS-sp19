/*:
 ## Exercise: Good Names
 
 You’ve inherited some code from another programmer, who never had a chance to take the _App Development with Swift_ course. All you found was a note with the following written on it:
 
 > **Truck loading guide**
 >
 > Pallets of oranges weigh 100lbs
 >
 > Pallets of watermelons weigh 200lbs
 >
 > Have to load up the left then right of the truck and keep it as balanced as possible. How many on each side? Melons always go together.
*/
let co = 14
let cw = 3
let ow = 100
let ww = 200
let to = co * ow
let tw = cw * ww
let ttl = to + tw
let es = ttl / 2
let lhso = es / ow
let rhso = co - lhso


//: - experiment: Rewrite the code so that it makes sense without needing a note. Use meaningful names and comments.
let numberOfOranges=14
let numberOfWatermelon=3
let orangeMass=100
let waterMelonMass=200
let totalMassOfOranges=numberOfOranges*orangeMass
let totalMassOfWatermelon=numberOfWatermelon*waterMelonMass
let totalMass=totalMassOfOranges+totalMassOfWatermelon
let totalMassOnEachSide=(totalMass/2)
let numberOfWatermelonOnEachSide=totalMassOnEachSide/orangeMass
let numberOfOrangesOnEachSide=numberOfWatermelonOnEachSide-numberOfOranges

//:[Previous](@previous)  |  page 14 of 14
